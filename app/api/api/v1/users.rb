      ...
      resource :users do
        ...
        post '/login' do
          begin
            ...
            if user = User.authenticate_for_api(params[:email], params[:password], firebase_access_token)              present user, with: API::V1::Entities::UserEntity, only: [:fullname, :email, :token]
            else
